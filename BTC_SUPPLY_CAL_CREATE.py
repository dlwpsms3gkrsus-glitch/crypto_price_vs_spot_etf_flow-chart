...
def get_full_bitcoin_supply_data():
    """CoinMetrics에서 전체 비트코인 총공급량 데이터를 가져옵니다."""
    print("▶ 1단계: 전체 비트코인 공급량 데이터를 다운로드합니다...")
    try:
        # 비트코인 제네시스 블록 생성일부터 전체 데이터를 요청합니다.
        start_date = "2009-01-03"
        url = "https://community-api.coinmetrics.io/v4/timeseries/asset-metrics"
        params = {
            "assets": "btc",  # 자산을 'btc'로 변경
            "metrics": "SplyCur",
...
if __name__ == "__main__":
    btc_supply_df = get_full_bitcoin_supply_data()

    if btc_supply_df is None:
        sys.exit("\n프로그램을 종료합니다.")

    # DataFrame을 JavaScript가 사용할 JSON 데이터로 변환
    data_json = btc_supply_df.to_json(orient='records')
    ...
    <head>
        ...
        <style>
            ...
            /* 비트코인 색상 테마 적용 */
            button {{ width: 100%; padding: 12px; font-size: 18px; font-weight: bold; color: white; background-color: #f7931a; border: none; border-radius: 6px; cursor: pointer; transition: background-color 0.3s; }}
            button:hover {{ background-color: #e68917; }}
            ...
            .result-item .value {{ font-weight: bold; color: #e68917; font-size: 1.2em; }}
        </style>
    </head>
    <body>
        ...
        <script>
            ...
            calculateBtn.addEventListener('click', () => {{
                ...
                document.getElementById('supply-increase-abs').textContent = `${{increaseAbs.toLocaleString(undefined, {{maximumFractionDigits: 4}})}} BTC`;
    ...
    # 3. HTML 파일로 저장
    file_name = "btc_supply_calculator.html"
